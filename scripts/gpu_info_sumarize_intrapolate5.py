...
def main():
    ...
    labels = {
            ...
            "exp2": ["Name", "Elapsed time (h)", "kWh", "CO2 (kg)"],
    ...
    for i in gpu_power:
        ...
        power[i].append(str(interpolated_values.values.mean(axis = 0).round(2))) # Get the mean / average
        gpu_hours = gputime[2]
        kwh = (interpolated_values.values.sum(axis = 0)/3600000).round(2)
        total_power += kwh
        power[i].append(str(kwh)) # GPU Hours
        power[i].append(str(round(PUE * kwh, 2))) # Effective GPU Hours
        power[i].append(str(PUE * carbon_intensity[args.region]['mean'] * kwh)) # to compute CO2
        power[i].append(str(PUE * carbon_intensity[args.region]['std'] * kwh)) # to compute deviation CO2
    ...
    #power_total.append((sum([gpu_power[i].sum(axis = 0)[1] for i in range(len(gpu_memory))])/sum([len(gpu_memory[i]) for i in gpu_memory])).round(2))
    x = np.arange(0, gpu_power[i].shape[0] * 5) # to get ranges from 0 till the end of step 5 (seconds)
    interpolated_values = [pd.Series([gpu_power[i][:, 1][v//5] if v%5 == 0 else np.nan for v in x]).interpolate() for i in gpu_power]
    power_total.append((sum([iv.values.sum(axis = 0) for iv in interpolated_values])/sum([len(iv) for iv in interpolated_values])).round(2)) # average fb over all gpus
    total_power = sum([iv.values.sum(axis = 0) for iv in interpolated_values]) # to convert to kWh
    print(str(total_power))
    total_power = (total_power/3600000).round(2) # to convert to kWh
    ...
    power_total.append(str(round(PUE * carbon_intensity[args.region]['std'] * total_power / 1000, 2))) # to compute deviation CO2
    power_total.append(sum([iv.values.mean(axis = 0) for iv in interpolated_values]).round(2))
    ...
    for i in gpu_memory:
        ...
        for ii in [1, 3, 4, 2]: # 1 = fb, 3 = SM, 4 = mem, 2 = temp
            ...
            x = np.arange(0, gpu_memory[i].shape[0] * 5) # to get ranges from 0 till the end of step 5 (seconds)
            interpolated_values = pd.Series([gpu_memory[i][:, ii][v//5] if v%5 == 0 else np.nan for v in x]).interpolate()
            memory[i].append(str(interpolated_values.values.mean(axis = 0).round(2))) # Get the mean / average

    
    for ii in [1, 3, 4, 2]: # 1 = fb, 3 = SM, 4 = mem, 2 = temp
        x = np.arange(0, gpu_memory[i].shape[0] * 5) # to get ranges from 0 till the end of step 5 (seconds)
        interpolated_values = [pd.Series([gpu_memory[i][:, ii][v//5] if v%5 == 0 else np.nan for v in x]).interpolate() for i in gpu_memory]
        memory_total.append((sum([iv.values.sum(axis = 0) for iv in interpolated_values])/sum([len(iv) for iv in interpolated_values])).round(2)) # average fb over all gpus
    ...
    if (args.experiment == '5' or args.experiment == ''):
        ...
        print(" & " + " & ".join([label for label in labels['exp5']]) + "\\\\\hline")
        print(expname + " & " + " & ".join([str(mt) for mt in memory_total]) + "\\\\\hline")
